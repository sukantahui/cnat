...
`;function U(){const c=s.useRef([]),[i,h]=s.useState([12,28,45,67,89]),[r,p]=s.useState(0),[y,l]=s.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,m]=s.useState("");s.useEffect(()=>{const t=new IntersectionObserver(a=>{a.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return c.current.forEach(a=>{a&&t.observe(a)}),()=>t.disconnect()},[]);const n=t=>{t&&!c.current.includes(t)&&c.current.push(t)},g=()=>{p(t=>(t+1)%i.length),l(`Advanced to step ${(r+1)%i.length}: Invariant verified at index ${(r+1)%i.length}.`)},x=()=>{const t=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(t)||(h(a=>[...a.slice(-7),t]),l(`Pushed value ${t} into active memory structure. Buffer state updated.`),m(""))},f=()=>{h([12,28,45,67,89]),p(0),l("Structure reset to base state."),m("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
