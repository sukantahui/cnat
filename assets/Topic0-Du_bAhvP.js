...
`;function O(){const l=r.useRef([]),[i,m]=r.useState([12,28,45,67,89]),[s,u]=r.useState(0),[g,c]=r.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,p]=r.useState("");r.useEffect(()=>{const t=new IntersectionObserver(n=>{n.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return l.current.forEach(n=>{n&&t.observe(n)}),()=>t.disconnect()},[]);const a=t=>{t&&!l.current.includes(t)&&l.current.push(t)},y=()=>{u(t=>(t+1)%i.length),c(`Advanced to step ${(s+1)%i.length}: Invariant verified at index ${(s+1)%i.length}.`)},x=()=>{const t=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(t)||(m(n=>[...n.slice(-7),t]),c(`Pushed value ${t} into active memory structure. Buffer state updated.`),p(""))},f=()=>{m([12,28,45,67,89]),u(0),c("Structure reset to base state."),p("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
