import{b as h,j as d,bg as w}from"./vendor-react-core-Doz9nIC6.js";const r={name:"Coder & AccoTax",phone:"700356860",website:"https://codernaccotax.co.in",email:"codernaccotax@gmail.com",address:"25(10/A) Shibtala Road, PO: NC Pukur, Barrackpore",teacher:"Sukanta Hui"},D=({filePath:b,content:x,buttonText:A="Print Note",title:m="Printable Note",className:C,stampEnabled:y=!0,stampColor:k="#4f46e5",stampText:$=r.name,showDownload:H=!0,downloadButtonText:N="Download TXT",downloadFileName:T})=>{const[c,j]=h.useState(x||""),[g,P]=h.useState(!1),[f,v]=h.useState(null);h.useEffect(()=>{if(x){j(x),v(null);return}if(!b){v("No file path or content provided.");return}(async()=>{P(!0),v(null);try{const n=await fetch(b);if(!n.ok)throw new Error(`HTTP ${n.status}: ${n.statusText}`);const t=await n.text();j(t)}catch(n){v(n.message||"Failed to load the file."),console.error("PlainTextPrint fetch error:",n)}finally{P(!1)}})()},[b,x]);const i=s=>s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),z=s=>{const n=i(s).split(`
