...
`;function O(){const l=r.useRef([]),[a,h]=r.useState([12,28,45,67,89]),[s,p]=r.useState(0),[g,c]=r.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,m]=r.useState("");r.useEffect(()=>{const t=new IntersectionObserver(n=>{n.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return l.current.forEach(n=>{n&&t.observe(n)}),()=>t.disconnect()},[]);const i=t=>{t&&!l.current.includes(t)&&l.current.push(t)},x=()=>{p(t=>(t+1)%a.length),c(`Advanced to step ${(s+1)%a.length}: Invariant verified at index ${(s+1)%a.length}.`)},y=()=>{const t=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(t)||(h(n=>[...n.slice(-7),t]),c(`Pushed value ${t} into active memory structure. Buffer state updated.`),m(""))},f=()=>{h([12,28,45,67,89]),p(0),c("Structure reset to base state."),m("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
