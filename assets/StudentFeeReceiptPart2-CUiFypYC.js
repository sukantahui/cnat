import{b as d,j as e}from"./vendor-react-core-BBXbtqt1.js";import{C as V}from"./cnat-Cw7eyaib.js";import{p as Y}from"./paid-stamp-DDbnMIS8.js";import{t as Ye,a as Qe}from"./vendor-canvas-e-rorcri.js";import{s as ye}from"./studentService-CfrU6gYV.js";import{c as be}from"./courseService-CZrLM58u.js";import{l as He}from"./loginService-DhyhgiWW.js";import{u as Je,D as Ke}from"./userService-9sz5jN98.js";import"./api-B-XEjutJ.js";const dt=()=>{const we=()=>{const t=new Date,a=t.getFullYear(),s=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0");return`${a}-${s}-${n}`},[u,ve]=d.useState(null),[Xe,Q]=d.useState(!0),[A,H]=d.useState([]),[F,J]=d.useState(!1),[Ne,T]=d.useState(""),[D,K]=d.useState(null),[Ze,N]=d.useState(""),[X,Z]=d.useState([]),[$,ee]=d.useState(!1),[R,te]=d.useState(null),[I,E]=d.useState(!1),[g,q]=d.useState({student_name:"",nickname:"",whatsapp:""}),[ae,re]=d.useState(!1),[M,h]=d.useState(null),[U,_]=d.useState(!1),[v,z]=d.useState({courseCode:"",courseName:""}),[se,oe]=d.useState(!1),[L,b]=d.useState(null),[r,f]=d.useState({studentName:"",phone:"",course:"",feeType:"non_monthly",monthlyFeeAmount:"",feesPaid:"",paymentDate:we(),paymentMode:"Cash",periodFrom:"",periodTo:"",registrationNumber:""}),[o,je]=d.useState(null),[j,ke]=d.useState(!1),[W,k]=d.useState(!1),[ne,ie]=d.useState(!1),le=d.useRef(null),[Ce,Se]=d.useState(""),[Pe,Ae]=d.useState(""),Fe=async()=>{Q(!0);try{const t=await He.currentUser();t&&t.status===!0&&t.data&&ve(t.data)}catch(t){console.error("Error fetching current user:",t)}finally{Q(!1)}};d.useEffect(()=>{Fe(),Te(),de()},[]);const Te=async()=>{J(!0),K(null);try{const t=await ye.getAll();let a=[];t&&t.data&&Array.isArray(t.data)?a=t.data:Array.isArray(t)?a=t:t&&t.data&&t.data.data&&Array.isArray(t.data.data)&&(a=t.data.data),H(a),a.length===0&&console.warn("No students found in database")}catch(t){console.error("Error fetching students:",t),K("Failed to load students from database. Please try again later."),H([])}finally{J(!1)}},de=async()=>{ee(!0),te(null);try{const t=await be.getAll();let a=[];t&&t.status===!0&&Array.isArray(t.data)?a=t.data:Array.isArray(t)?a=t:t&&t.data&&Array.isArray(t.data)&&(a=t.data),Z(a),a.length===0&&console.warn("No courses found in database")}catch(t){console.error("Error fetching courses:",t),te("Failed to load courses from database. Please try again later."),Z([])}finally{ee(!1)}},ce=()=>u?u.employee&&u.employee.employeeName?u.employee.employeeName:u.userName?u.userName:"Authorized Collector":"System",pe=()=>u?u.employee&&u.employee.designation?u.employee.designation.name:u.role||u.userType&&u.userType.userTypeName?u.role||u.userType.userTypeName:"Accounts Department":"",B=t=>{const{name:a,value:s}=t.target;q(n=>({...n,[a]:s})),M&&h(null)},De=async t=>{if(t.preventDefault(),!g.student_name.trim()){h("Student name is required");return}if(!g.whatsapp.trim()){h("WhatsApp number is required");return}if(!/^\d{10}$/.test(g.whatsapp)){h("Please enter a valid 10-digit WhatsApp number");return}re(!0),h(null);try{const s={student_name:g.student_name.trim(),nickname:g.nickname.trim()||g.student_name.trim(),whatsapp:g.whatsapp.trim()},n=await ye.createBasic(s);if(n&&n.status===!0){const i=n.data,m=i?.id||i?.studentId,c=i?.studentName||i?.student_name||"",p=i?.registrationNumber||i?.regNo||i?.reg_no||"";if(i&&m){T(m.toString()),N(p),f(l=>({...l,studentName:c,phone:i.whatsapp||i.phone1||"",registrationNumber:p}));try{await Je.createStudentUser({id:m,student_name:c,whatsapp:g.whatsapp.trim(),enrollment_number:p,registration_number:p},Ke)}catch(l){console.warn("Auto student user creation notice:",l)}}q({student_name:"",nickname:"",whatsapp:""}),E(!1),alert(`Student "${c}" created successfully!
Registration Number: ${p}`)}else h(n?.message||"Failed to create student")}catch(s){console.error("Error creating student:",s),h(s.response?.data?.message||"Failed to create student. Please try again.")}finally{re(!1)}},me=t=>{const{name:a,value:s}=t.target;z(n=>({...n,[a]:s})),L&&b(null)},$e=async t=>{if(t.preventDefault(),!v.courseCode.trim()){b("Course code is required");return}if(!v.courseName.trim()){b("Course name is required");return}oe(!0),b(null);try{const a={courseCode:v.courseCode.trim().toUpperCase(),courseName:v.courseName.trim()},s=await be.createBasic(a);if(s&&s.status===!0){const n=s.data;await de(),n&&n.id&&f(i=>({...i,course:n.courseName||""})),z({courseCode:"",courseName:""}),_(!1),alert(`Course "${n.courseName}" created successfully!
Course Code: ${n.courseCode}`)}else b(s?.message||"Failed to create course")}catch(a){console.error("Error creating course:",a),b(a.response?.data?.message||"Failed to create course. Please try again.")}finally{oe(!1)}};d.useEffect(()=>{(async()=>{try{const s=await(await fetch(V)).blob(),n=new FileReader;n.onloadend=()=>{Se(n.result)},n.readAsDataURL(s);const m=await(await fetch(Y)).blob(),c=new FileReader;c.onloadend=()=>{Ae(c.result)},c.readAsDataURL(m)}catch(a){console.error("Error loading images:",a)}})()},[]);const Re=t=>{const a=t.target.value;if(T(a),a){const s=A.find(n=>n.studentId===parseInt(a));s&&(N(s.registrationNumber||""),f(n=>({...n,studentName:s.studentName||"",phone:s.whatsapp||s.phone1||"",registrationNumber:s.registrationNumber||""})))}else N(""),f(s=>({...s,studentName:"",phone:"",registrationNumber:""}))},Ie=(t,a,s,n)=>{const i="CNAT",m=`${t.trim()}_${a}_${s}_${n}`,p=(fe=>{let w=0;for(let G=0;G<fe.length;G++){const Ve=fe.charCodeAt(G);w=(w<<5)-w+Ve,w=w&w}return Math.abs(w).toString(36).toUpperCase()})(m),l=new Date,S=l.getFullYear(),y=String(l.getMonth()+1).padStart(2,"0"),P=String(l.getDate()).padStart(2,"0"),Oe=`${S}${y}${P}`,Ge=p.substring(0,8);return`${i}-${Oe}-${Ge}`},x=t=>{const{name:a,value:s}=t.target;f(n=>({...n,[a]:s})),(a==="studentName"||a==="phone")&&(T(""),N(""),f(n=>({...n,registrationNumber:""})))},C=(t,a)=>{if(!t||!a)return 0;const s=new Date(t),n=new Date(a),i=(n.getFullYear()-s.getFullYear())*12+(n.getMonth()-s.getMonth())+1;return i>0?i:0},ue=t=>{const a=t.target.value;f(s=>({...s,feeType:a,feesPaid:"",periodFrom:"",periodTo:"",monthlyFeeAmount:""}))},Ee=()=>{if(r.feeType==="monthly"){const t=C(r.periodFrom,r.periodTo),a=parseFloat(r.monthlyFeeAmount)||0,s=t*a;f(n=>({...n,feesPaid:s.toString()}))}};d.useEffect(()=>{r.feeType==="monthly"&&r.periodFrom&&r.periodTo&&r.monthlyFeeAmount&&Ee()},[r.periodFrom,r.periodTo,r.monthlyFeeAmount,r.feeType]);const ge=t=>{if(!t||isNaN(t))return"";if(t=Math.floor(t),t===0)return"Zero Only";const a=["","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"],s=["","","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"],n=["Ten","Eleven","Twelve","Thirteen","Fourteen","Fifteen","Sixteen","Seventeen","Eighteen","Nineteen"],i=l=>l===0?"":l<10?a[l]:l<20?n[l-10]:s[Math.floor(l/10)]+(l%10?" "+a[l%10]:""),m=l=>{if(l===0)return"";if(l<100)return i(l);const S=Math.floor(l/100),y=l%100;return a[S]+" Hundred"+(y?" and "+i(y):"")};let c="",p=t;if(p>=1e7){const l=Math.floor(p/1e7);c+=i(l)+" Crore",p%=1e7,p>0&&(c+=" ")}if(p>=1e5){const l=Math.floor(p/1e5);c+=i(l)+" Lakh",p%=1e5,p>0&&(c+=" ")}if(p>=1e3){const l=Math.floor(p/1e3);c+=m(l)+" Thousand",p%=1e3,p>0&&(c+=" ")}return p>0&&(c+=m(p)),c+" Only"},qe=t=>t?new Date(t).toLocaleDateString("en-IN",{day:"2-digit",month:"long",year:"numeric"}):"",xe=t=>t?new Date(t).toLocaleDateString("en-IN",{month:"long",year:"numeric"}):"",he=t=>{switch(t){case"Cash":return"💵";case"Online (UPI)":return"📱";case"Online (Card)":return"💳";case"Online (Net Banking)":return"🏦";case"Cheque":return"📝";default:return"💰"}},O=async(t="jpeg")=>{const a=le.current;if(!a)throw new Error("Receipt element not found");const s={overflow:a.style.overflow,width:a.style.width,position:a.style.position};a.style.overflow="visible",a.style.width="fit-content",a.style.position="relative";const n=a.scrollWidth,i=a.scrollHeight,m={quality:1,pixelRatio:3,backgroundColor:"#ffffff",cacheBust:!0,width:n,height:i,style:{margin:"0",padding:"0",transform:"none",overflow:"visible"},skipAutoScale:!1,preferDimensions:!0};let c;return t==="jpeg"?c=await Ye(a,m):c=await Qe(a,m),a.style.overflow=s.overflow,a.style.width=s.width,a.style.position=s.position,c},Me=async()=>{if(!o){alert("Please generate a receipt first");return}const t=r.phone;if(!t){alert("Student phone number is required to send via WhatsApp");return}let a=t.replace(/\D/g,"");a.length===10&&(a="91"+a),!a.startsWith("91")&&a.length===12&&(a=a.substring(0,2)==="91"?a:"91"+a),ie(!0);try{const s=await O("jpeg"),i=await(await fetch(s)).blob(),m=new File([i],`Receipt_${o.receiptNo}.jpg`,{type:"image/jpeg"}),c=`📄 *Fee Payment Receipt - Coder & AccoTax* 📄
...
For any queries, contact: +91 70037 56860`,p=encodeURIComponent(c),l=`https://wa.me/${a}?text=${p}`;if(window.open(l,"_blank"),window.confirm(`WhatsApp will open now. Please follow these steps:
        ...
        <div class="receipt-container">
          <div class="receipt">
            ...
            <div class="watermark-container">
              <img src="${m}" alt="Watermark" class="watermark-image" />
            </div>
            ...
            <div class="receipt-content">
              <div class="header">
                <div class="organisation-name">
                  <img src="${m}" alt="Coder & AccoTax Logo" class="organisation-logo" />
