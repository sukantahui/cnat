...
`;function P(){const l=s.useRef([]),[a,u]=s.useState([12,28,45,67,89]),[r,p]=s.useState(0),[g,c]=s.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,m]=s.useState("");s.useEffect(()=>{const n=new IntersectionObserver(t=>{t.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return l.current.forEach(t=>{t&&n.observe(t)}),()=>n.disconnect()},[]);const i=n=>{n&&!l.current.includes(n)&&l.current.push(n)},y=()=>{p(n=>(n+1)%a.length),c(`Advanced to step ${(r+1)%a.length}: Invariant verified at index ${(r+1)%a.length}.`)},x=()=>{const n=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(n)||(u(t=>[...t.slice(-7),n]),c(`Pushed value ${n} into active memory structure. Buffer state updated.`),m(""))},f=()=>{u([12,28,45,67,89]),p(0),c("Structure reset to base state."),m("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
