...
WhatsApp **+91 94324 56083** or Call **+91 70037 56860**.`}function $(){return`### 📞 Coder & AccoTax Official Contact & Helpline
...
${E()}`:P();if(e.includes("course")||e.includes("courses")||e.includes("curriculum")||e.includes("syllabus")||e.includes("programs")||e.includes("what do you teach")||e.includes("what courses")||e.includes("all courses")||e.includes("courses offered")||e.includes("training tracks")||e.includes("degrees")||e.includes("diploma")||e.includes("certification")||e.includes("certifications"))return E();if(e.includes("helpline")||e.includes("whatsapp number")||e.includes("phone number")||e.includes("mobile number")||e.includes("call")||e.includes("contact number"))return $();let h=null,l=0;for(const f of pe){let g=0;for(const C of f.keywords)e===C?g+=15:e.includes(C)&&(g+=C.length>4?6:3);g>l&&(l=g,h=f)}if(h&&l>=5)return h.generate(t);if(e.includes("explain")||e.includes("summary")||e.includes("what is this")||e.includes("help")||e.includes("how to")||e.includes("concept")||e.includes("example")||e.includes("tutorial")||e.includes("notes"))return`### 💡 Mentor Guide for: ${s}
...
*Need practice questions?* Check the interactive Practice Questions and Quiz engine integrated at the bottom of this topic page!`;const d=de(o,t);return d||`### 🎓 CNAT Mam Academic Assistance

Dear student, you asked: **"${i}"** regarding **${s}**.

Here is my mentorship guidance:

- **Key Focus Area:** When studying **${s}** in our **${p.toUpperCase()}** curriculum, always make sure you understand the underlying computational or arithmetic mechanism before writing code or formulas.
...
*You can also click any of the quick prompt chips below to explore specific formulas, code patterns, teacher details, or course roadmaps!*`}function be({topicTitle:o="Study Topic",moduleSlug:t="",subjectKey:i="excel"}){const[e,s]=b.useState(!1),[p,h]=b.useState(!1),[l,d]=b.useState(""),[f,g]=b.useState(!1),[C,w]=b.useState(!1),[S,k]=b.useState(null),[M,O]=b.useState(null),[j,L]=b.useState(()=>{try{return localStorage.getItem("cnat_mam_assistant_closed")==="true"}catch{return!1}}),F=r=>{r?.stopPropagation?.(),L(!0);try{localStorage.setItem("cnat_mam_assistant_closed","true")}catch{}},_=()=>{L(!1),s(!0);try{localStorage.removeItem("cnat_mam_assistant_closed")}catch{}},W=me(i),[R,I]=b.useState([{sender:"bot",text:`Hello dear student! I am **CNAT Mam**, your senior AI academic mentor at **Coder & AccoTax**.

I can guide you through **"${o}"**, explain formulas and code, provide our **campus address**, **teachers & mentors details**, or **course syllabi & admissions**. How can I help you today?`,time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}]),B=b.useRef(null),U=()=>{B.current?.scrollIntoView({behavior:"smooth"})};b.useEffect(()=>{e&&U()},[R,e,f]);const H=(r,c)=>{if(typeof window>"u"||!("speechSynthesis"in window))return;if(C&&S===c){window.speechSynthesis.cancel(),w(!1),k(null);return}window.speechSynthesis.cancel();const n=r.replace(/[*#`_~[\]()>-]/g," ").replace(/\|/g," ").replace(/\s+/g," ").trim(),u=new SpeechSynthesisUtterance(n);u.rate=1,u.pitch=1.05;const v=window.speechSynthesis.getVoices(),x=v.find(y=>y.lang.includes("en")&&(y.name.includes("Female")||y.name.includes("Google")||y.name.includes("Samantha")||y.name.includes("Zira")))||v.find(y=>y.lang.includes("en"));x&&(u.voice=x),u.onend=()=>{w(!1),k(null)},u.onerror=()=>{w(!1),k(null)},w(!0),k(c),window.speechSynthesis.speak(u)};b.useEffect(()=>{!e&&typeof window<"u"&&"speechSynthesis"in window&&(window.speechSynthesis.cancel(),w(!1),k(null))},[e]);const A=r=>{const c=r||l;if(!c.trim())return;const n=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),u={sender:"user",text:c,time:n};I(v=>[...v,u]),r||d(""),g(!0),setTimeout(()=>{const v=he(c,{topicTitle:o,moduleSlug:t,subjectKey:i}),x=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});I(y=>[...y,{sender:"bot",text:v,time:x}]),g(!1)},450)},G=(r,c)=>{navigator.clipboard.writeText(r),O(c),setTimeout(()=>O(null),1500)},z=()=>{typeof window<"u"&&"speechSynthesis"in window&&(window.speechSynthesis.cancel(),w(!1),k(null)),I([{sender:"bot",text:`Chat reset! I am ready for your next question regarding **"${o}"**, institute address, teachers details, or courses.`,time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}])};return a.jsxs("div",{className:"fixed bottom-5 right-5 z-50 font-sans selection:bg-sky-500/30 selection:text-sky-200",children:[j&&!e&&a.jsx("button",{type:"button",onClick:_,className:"group flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-slate-900/90 hover:bg-slate-800 border border-sky-500/40 text-sky-400 shadow-xl backdrop-blur transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer",title:"Open AI Assistant (Ask CNAT Mam)","aria-label":"Open AI Assistant",children:a.jsx(Q,{size:16,className:"text-teal-400 group-hover:rotate-12 transition-transform"})}),!j&&!e&&a.jsxs(a.Fragment,{children:[a.jsxs("div",{className:"flex sm:hidden items-center gap-1.5 bg-gradient-to-r from-indigo-600 via-sky-600 to-teal-600 pl-2 pr-1.5 py-1.5 rounded-full shadow-2xl border border-sky-400/30",children:[a.jsxs("button",{type:"button",onClick:()=>s(!0),className:"flex items-center gap-1.5 text-white font-bold text-xs active:scale-95 transition-all cursor-pointer",children:[a.jsx("div",{className:"w-5 h-5 rounded-full overflow-hidden border border-white/40 p-0.5 bg-slate-900 flex-shrink-0",children:a.jsx("img",{src:"/teachers/cnat_mam.jpg",alt:"CNAT Mam",className:"w-full h-full rounded-full object-cover",onError:r=>{r.target.style.display="none"}})}),a.jsx("span",{className:"whitespace-nowrap",children:"Ask AI"})]}),a.jsx("button",{type:"button",onClick:F,className:"w-5 h-5 rounded-full bg-black/30 hover:bg-black/60 text-white/80 hover:text-white flex items-center justify-center text-[10px] transition cursor-pointer",title:"Close AI Assistant","aria-label":"Close AI Assistant",children:a.jsx(D,{size:11})})]}),a.jsxs("div",{className:"hidden sm:flex items-center gap-2 bg-gradient-to-r from-indigo-600 via-sky-600 to-teal-600 pl-3.5 pr-2 py-2 rounded-2xl shadow-2xl border border-sky-400/30",children:[a.jsxs("button",{type:"button",onClick:()=>s(!0),className:"flex items-center gap-2.5 text-white font-bold text-sm hover:opacity-95 transition cursor-pointer",children:[a.jsxs("span",{className:"relative flex h-2.5 w-2.5",children:[a.jsx("span",{className:"animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"}),a.jsx("span",{className:"relative inline-flex rounded-full h-2.5 w-2.5 bg-teal-300"})]}),a.jsxs("div",{className:"flex items-center gap-2",children:[a.jsx("div",{className:"w-7 h-7 rounded-full overflow-hidden border border-sky-300/40 p-0.5 shadow-sm bg-slate-900",children:a.jsx("img",{src:"/teachers/cnat_mam.jpg",alt:"CNAT Mam",className:"w-full h-full rounded-full object-cover",onError:r=>{r.target.style.display="none"}})}),a.jsx("span",{children:"Ask CNAT Mam"})]}),a.jsx("span",{className:"px-2 py-0.5 rounded-md bg-white/20 text-[10px] uppercase font-extrabold tracking-wider text-teal-100",children:"AI Mentor"})]}),a.jsx("button",{type:"button",onClick:F,className:"w-6 h-6 rounded-full bg-black/20 hover:bg-black/40 text-white/70 hover:text-white flex items-center justify-center transition cursor-pointer",title:"Close AI Assistant","aria-label":"Close AI Assistant",children:a.jsx(D,{size:13})})]})]}),e&&a.jsxs("div",{className:`${p?"w-[94vw] sm:w-[680px] h-[720px]":"w-[92vw] sm:w-[440px] h-[600px]"} max-h-[88vh] rounded-3xl bg-slate-950/95 border border-slate-800 shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-300 border-sky-500/30 transition-all`,children:[a.jsxs("div",{className:"px-4 py-3.5 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border-b border-slate-800 flex items-center justify-between",children:[a.jsxs("div",{className:"flex items-center gap-3",children:[a.jsxs("div",{className:"relative",children:[a.jsx("div",{className:"w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 via-sky-400 to-teal-400 p-0.5 shadow-md overflow-hidden",children:a.jsx("img",{src:"/teachers/cnat_mam.jpg",alt:"CNAT Mam Profile",className:"w-full h-full rounded-full object-cover",onError:r=>{r.target.style.display="none"}})}),a.jsx("span",{className:"absolute bottom-0 right-0 w-3 h-3 rounded-full bg-emerald-500 border-2 border-slate-900"})]}),a.jsxs("div",{children:[a.jsxs("div",{className:"flex items-center gap-2",children:[a.jsx("h3",{className:"font-extrabold text-white text-sm tracking-wide",children:"CNAT Mam"}),a.jsxs("span",{className:"px-1.5 py-0.5 rounded bg-sky-950 border border-sky-700 text-sky-300 text-[10px] font-mono font-semibold",children:[i.toUpperCase()," Mentor"]})]}),a.jsx("p",{className:"text-[11px] text-slate-400 truncate max-w-[240px]",children:o})]})]}),a.jsxs("div",{className:"flex items-center gap-1 text-slate-400",children:[a.jsx("button",{onClick:()=>h(r=>!r),title:p?"Compact View":"Expand View",className:"p-1.5 rounded-lg hover:bg-slate-800 hover:text-white transition cursor-pointer",children:p?a.jsx(J,{size:15}):a.jsx(K,{size:15})}),a.jsx("button",{onClick:z,title:"Reset Conversation",className:"p-1.5 rounded-lg hover:bg-slate-800 hover:text-white transition cursor-pointer",children:a.jsx(X,{size:15})}),a.jsx("button",{onClick:()=>s(!1),title:"Close Chat",className:"p-1.5 rounded-lg hover:bg-slate-800 hover:text-white transition cursor-pointer",children:a.jsx(D,{size:18})})]})]}),a.jsxs("div",{className:"px-3 py-1.5 bg-slate-900/95 border-b border-slate-800/80 flex items-center justify-between text-[11px] text-slate-300",children:[a.jsxs("div",{className:"flex items-center gap-2",children:[a.jsxs("button",{onClick:()=>A("What is the official address and campus location of Coder & AccoTax?"),className:"flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-800 hover:bg-sky-950 border border-slate-700 hover:border-sky-600 text-sky-300 hover:text-sky-100 transition cursor-pointer",title:"View Campus Address",children:[a.jsx(Y,{size:11,className:"text-sky-400"}),a.jsx("span",{children:"Address"})]}),a.jsxs("button",{onClick:()=>A("Who are the teachers and faculty members at Coder & AccoTax?"),className:"flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-800 hover:bg-indigo-950 border border-slate-700 hover:border-indigo-600 text-indigo-300 hover:text-indigo-100 transition cursor-pointer",title:"View Faculty Profiles",children:[a.jsx(Z,{size:11,className:"text-indigo-400"}),a.jsx("span",{children:"Teachers"})]}),a.jsxs("button",{onClick:()=>A("What are all the courses offered at Coder & AccoTax?"),className:"flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-800 hover:bg-emerald-950 border border-slate-700 hover:border-emerald-600 text-emerald-300 hover:text-emerald-100 transition cursor-pointer",title:"View Courses Catalog",children:[a.jsx(ee,{size:11,className:"text-emerald-400"}),a.jsx("span",{children:"Courses"})]})]}),a.jsx("span",{className:"text-[10px] text-slate-400 hidden sm:inline",children:"Coder & AccoTax"})]}),a.jsxs("div",{className:"flex-1 overflow-y-auto p-4 space-y-4 text-xs sm:text-sm custom-scrollbar bg-slate-950/40",children:[R.map((r,c)=>a.jsxs("div",{className:`flex gap-2.5 ${r.sender==="user"?"justify-end":"justify-start"}`,children:[r.sender==="bot"&&a.jsx("img",{src:"/teachers/cnat_mam.jpg",alt:"CNAT Mam",className:"w-7 h-7 rounded-full object-cover border border-sky-600/60 shrink-0 mt-0.5 shadow-sm",onError:n=>{n.target.style.display="none"}}),a.jsxs("div",{className:`relative max-w-[88%] rounded-2xl px-4 py-3 shadow-md text-xs sm:text-sm leading-relaxed ${r.sender==="user"?"bg-gradient-to-r from-sky-600 to-indigo-600 text-white rounded-br-none":"bg-slate-900 border border-slate-800 text-slate-200 rounded-bl-none"}`,children:[a.jsx("div",{className:"whitespace-pre-wrap font-sans space-y-2",children:r.text.split(`
`).map((n,u)=>{if(n.trim()==="---")return a.jsx("hr",{className:"my-2 border-slate-800"},u);if(n.startsWith("### "))return a.jsx("h4",{className:"font-bold text-sky-300 text-sm mt-2 mb-1",children:n.replace("### ","")},u);if(n.startsWith("#### "))return a.jsx("h5",{className:"font-semibold text-teal-300 text-xs mt-1.5 mb-0.5",children:n.replace("#### ","")},u);if(n.startsWith("> "))return a.jsx("div",{className:"my-1.5 p-3 rounded-xl bg-slate-950 border-l-4 border-sky-500 text-slate-200 font-sans text-xs",children:T(n.replace(/^>\s*/,""))},u);if(n.startsWith("`=")||n.startsWith("`")||n.startsWith("```")){const x=n.replace(/```[a-z]*|```/g,"").replace(/`/g,"");return x.trim()?a.jsx("div",{className:"my-1.5 p-2 rounded-xl bg-slate-950 border border-slate-800 font-mono text-sky-300 text-xs overflow-x-auto",children:x},u):null}if(n.trim().startsWith("- ")||n.trim().startsWith("* ")){const x=n.trim().slice(2);return a.jsxs("div",{className:"flex items-start gap-1.5 pl-1",children:[a.jsx("span",{className:"text-sky-400 mt-0.5",children:"•"}),a.jsx("span",{children:T(x)})]},u)}const v=n.trim().match(/^(\d+)\.\s+(.*)$/);if(v)return a.jsxs("div",{className:"flex items-start gap-1.5 pl-1",children:[a.jsxs("span",{className:"font-bold text-amber-400 mt-0.5 text-xs",children:[v[1],"."]}),a.jsx("span",{children:T(v[2])})]},u);if(n.trim().startsWith("|")&&n.trim().endsWith("|")){if(n.includes(":---")||n.includes("---"))return null;const x=n.split("|").filter(y=>y.trim()!=="");return a.jsx("div",{className:"my-1 flex gap-2 overflow-x-auto text-[11px] bg-slate-950/60 p-1.5 rounded-lg border border-slate-800",children:x.map((y,V)=>a.jsx("div",{className:"flex-1 font-mono text-slate-300",children:T(y.trim())},V))},u)}return a.jsx("p",{children:T(n)},u)})}),a.jsxs("div",{className:"mt-2.5 pt-1.5 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-400",children:[a.jsx("span",{children:r.time}),r.sender==="bot"&&a.jsxs("div",{className:"flex items-center gap-2",children:[a.jsxs("button",{onClick:()=>H(r.text,c),className:`hover:text-sky-300 transition flex items-center gap-1 cursor-pointer ${C&&S===c?"text-amber-400 animate-pulse font-bold":""}`,title:"Read Aloud",children:[C&&S===c?a.jsx(ae,{size:13}):a.jsx(te,{size:13}),a.jsx("span",{children:C&&S===c?"Stop":"Listen"})]}),a.jsxs("button",{onClick:()=>G(r.text,c),className:"hover:text-sky-300 transition flex items-center gap-1 cursor-pointer",title:"Copy Answer",children:[M===c?a.jsx(re,{size:12,className:"text-emerald-400"}):a.jsx(se,{size:12}),a.jsx("span",{children:M===c?"Copied":"Copy"})]})]})]})]}),r.sender==="user"&&a.jsx("div",{className:"w-7 h-7 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 shrink-0 mt-0.5",children:a.jsx(ne,{size:14})})]},c)),f&&a.jsxs("div",{className:"flex items-center gap-2 text-slate-400 text-xs italic pl-9",children:[a.jsxs("span",{className:"flex gap-1",children:[a.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-sky-400 animate-ping"}),a.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-sky-400 animate-ping delay-100"}),a.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-sky-400 animate-ping delay-200"})]}),a.jsx("span",{children:"CNAT Mam is preparing your response..."})]}),a.jsx("div",{ref:B})]}),a.jsx("div",{className:"px-3 py-2 bg-slate-900/90 border-t border-slate-800/80 overflow-x-auto flex gap-1.5 no-scrollbar",children:W.map((r,c)=>a.jsx("button",{onClick:()=>A(r.query),className:"px-2.5 py-1 rounded-full bg-slate-800 hover:bg-sky-950 border border-slate-700 hover:border-sky-700 text-slate-300 hover:text-sky-200 text-[11px] font-medium whitespace-nowrap transition-all duration-200 shrink-0 cursor-pointer",children:r.label},c))}),a.jsxs("form",{onSubmit:r=>{r.preventDefault(),A()},className:"p-3 bg-slate-950 border-t border-slate-800 flex items-center gap-2",children:[a.jsx("input",{type:"text",value:l,onChange:r=>d(r.target.value),placeholder:`Ask about ${o}, address, teachers, courses...`,className:"flex-1 px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-100 placeholder:text-slate-500 text-xs sm:text-sm focus:outline-none focus:border-sky-500/60 focus:ring-1 focus:ring-sky-500/60 transition"}),a.jsx("button",{type:"submit",disabled:!l.trim(),className:"p-2.5 rounded-xl bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white disabled:opacity-40 disabled:cursor-not-allowed transition shadow-md shrink-0 cursor-pointer",title:"Send Question",children:a.jsx(oe,{size:16})})]})]})]})}function T(o){return o?o.split(/(\*\*.*?\*\*|`.*?`|\[.*?\]\(.*?\))/g).map((i,e)=>{if(i.startsWith("**")&&i.endsWith("**"))return a.jsx("strong",{className:"font-bold text-amber-300",children:i.slice(2,-2)},e);if(i.startsWith("`")&&i.endsWith("`"))return a.jsx("code",{className:"px-1.5 py-0.5 rounded bg-slate-950 border border-slate-800 font-mono text-sky-300 text-[11px]",children:i.slice(1,-1)},e);const s=i.match(/^\[(.*?)\]\((.*?)\)$/);return s?a.jsx("a",{href:s[2],target:"_blank",rel:"noopener noreferrer",className:"text-sky-400 font-semibold underline hover:text-sky-300 transition",children:s[1]},e):i}):null}export{be as default};
