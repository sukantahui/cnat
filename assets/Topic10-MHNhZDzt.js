import{b as n,j as e,bm as r}from"./vendor-react-core-BBXbtqt1.js";import{T as y}from"./TeacherSukantaHui-VMH_OtYp.js";import{F as A}from"./FAQTemplate-BamsZuNW.js";import{P as v}from"./PlainTextPrint-CGJ_aUDk.js";import{P as k}from"./PythonFileLoader-YugaCzY9.js";import"./vendor-icons-IjXdH6hS.js";import"./PythonCodeBlock-BDgVOCqn.js";import"./vendor-prism-B7oW9GGR.js";const I=[{id:1,question:"What is the primary architectural difference between SAML 2.0, OAuth 2.0, and OpenID Connect (OIDC)?",shortAnswer:"SAML 2.0: Enterprise XML-based protocol for user authentication and Web Browser SSO. OAuth 2.0: Delegated authorization framework for API resource access. OpenID Connect (OIDC): Identity/Authentication layer built directly on top of OAuth 2.0 using JSON and JWTs.",explanation:"OAuth 2.0 grants API access (authorization); OIDC asserts user identity (authentication); SAML federates enterprise web logins.",hint:"SAML = Enterprise XML SSO; OAuth = API Authorization; OIDC = Identity layer over OAuth using JWTs.",level:"Basic",codeExample:`// Protocol Essence:
...
// [Central IdP] ➔ OIDC + PKCE ➔ [Modern Cloud Apps] ➔ DPoP Tokens ➔ [APIs]`}],w=`====================================================================================================\r
ACADEMIC STUDY GUIDE & FORENSIC NOTES: SSO & FEDERATED IDENTITY PROTOCOLS (SAML, OAUTH 2.0 & OIDC)\r
COURSE MODULE: 005_005 - AUTHENTICATION FRAMEWORKS & MFA (TOPIC 10)\r
INSTITUTION: CODER & ACCOTAX | INSTRUCTOR: SUKANTA HUI | LOCATION: BARRACKPORE, WEST BENGAL\r
====================================================================================================\r
\r
1. THE FEDERATED IDENTITY LANDSCAPE: SAML VS OAUTH VS OIDC\r
----------------------------------------------------------------------------------------------------\r
Understanding the distinct architectural roles of the three dominant identity protocols:\r
\r
1. SAML 2.0 (Security Assertion Markup Language):\r
   - Purpose: Enterprise Federation & Web Browser Single Sign-On (SSO).\r
   - Data Format: XML with XML Signatures (DSig) and XML Encryption.\r
   - Roles: Identity Provider (IdP e.g., Okta / Entra ID) and Service Provider (SP e.g., Salesforce).\r
   - Mechanism: Browser redirects carrying \`<samlp:AuthnRequest>\` and base64-encoded \`<samlp:Response>\`.\r
\r
2. OAuth 2.0 (RFC 6749):\r
   - Purpose: Delegated Authorization (API Access Delegation).\r
   - Key Principle: "OAuth is NOT an authentication protocol!" It issues Access Tokens granting \r
     permission to access protected resources on behalf of a user.\r
   - Tokens: Access Tokens (Bearer) and Refresh Tokens.\r
\r
3. OpenID Connect 1.0 (OIDC):\r
   - Purpose: Identity & User Authentication layer built directly on top of OAuth 2.0.\r
   - Data Format: JSON & JSON Web Tokens (JWT).\r
   - Roles: OpenID Provider (OP) and Relying Party (RP).\r
   - Introduction: Introduces the \`ID Token\` (JWT asserting user identity, email, and authentication time).\r
\r
\r
2. OAUTH 2.0 / OIDC AUTHORIZATION CODE FLOW WITH PKCE (RFC 7636)\r
----------------------------------------------------------------------------------------------------\r
Mandatory for all Single Page Apps (SPAs), Mobile Apps, and confidential web clients:\r
\r
The PKCE (Proof Key for Code Exchange) Flow:\r
1. Client generates a cryptographically random \`code_verifier\` (43–128 URL-safe characters).\r
2. Client computes \`code_challenge = BASE64URL(SHA-256(code_verifier))\`.\r
3. Client redirects user to Authorization Server with \`code_challenge\` and \`code_challenge_method=S256\`.\r
4. User authenticates; Authorization Server returns an ephemeral \`authorization_code\`.\r
5. Client sends \`authorization_code\` + original \`code_verifier\` to \`/token\` endpoint.\r
6. Authorization Server computes SHA-256 on the verifier; if it matches the stored challenge, it issues \r
   the \`ID Token\` and \`Access Token\`.\r
7. Defense: Prevents malicious apps on the device from intercepting and redeeming the authorization code!\r
\r
\r
3. JSON WEB TOKEN (JWT) RFC 7519 ARCHITECTURE\r
----------------------------------------------------------------------------------------------------\r
Structure: \`Header.Payload.Signature\` (Base64URL encoded)\r
\r
1. Header:\r
   - \`{"alg": "RS256", "typ": "JWT", "kid": "key_2026_01"}\`\r
2. Payload (Claims):\r
   - \`iss\` (Issuer): URL of the issuing identity provider.\r
   - \`sub\` (Subject): Unique immutable user ID.\r
   - \`aud\` (Audience): Client ID of the application intended to receive this token.\r
   - \`exp\` (Expiration Time): Unix timestamp after which token is invalid.\r
   - \`iat\` (Issued At): Unix timestamp of generation.\r
   - \`nonce\`: Cryptographic random string preventing replay attacks.\r
3. Signature:\r
   - $\\text{Signature} = \\text{HMAC-SHA256}(\\text{Header} \\ || \\ \\text{Payload}, \\text{Secret})$ or $\\text{RSA-SHA256}$.\r
\r
\r
4. TOP PROTOCOL VULNERABILITIES & FORENSIC EXPLOITS\r
----------------------------------------------------------------------------------------------------\r
1. XML Signature Wrapping (XSW) in SAML:\r
   - Adversary modifies the XML DOM tree, inserting an unsigned rogue admin Assertion while keeping the \r
     original signed assertion. If the signature validator and business logic inspect different elements, \r
     privilege escalation occurs.\r
   - Defense: Strict schema validation and absolute XPath element binding.\r
\r
2. JWT "alg: none" Vulnerability:\r
   - Attacker alters JWT header to \`{"alg": "none"}\`, strips the signature, and accesses admin endpoints.\r
   - Defense: Hardcode permitted signature algorithms on the backend; reject \`none\` unconditionally.\r
\r
3. JWT Key Confusion (Algorithm Switching: RS256 → HS256):\r
   - Attacker changes \`alg\` from RS256 (asymmetric) to HS256 (symmetric) and signs the token using the \r
     server's public RSA key as the HMAC secret!\r
   - Defense: Never use the same verification function for both symmetric and asymmetric keys.\r
\r
\r
5. REGIONAL SOC CASE STUDIES (WEST BENGAL DEPLOYMENTS)\r
----------------------------------------------------------------------------------------------------\r
Case 1: Barrackpore Municipal Treasury SAML XSW Remediation\r
- Threat: Penetration testing revealed SAML response parsing vulnerability in legacy vendor portal.\r
- Remediation: Enforced XML Signature Reference ID validation and moved to OIDC PKCE.\r
\r
Case 2: Salt Lake Sector V FinTech OAuth 2.0 PKCE Hardening\r
- Threat: Public mobile banking apps were susceptible to authorization code interception.\r
- Remediation: Enforced RFC 7636 PKCE S256 across all Android and iOS client endpoints.\r
====================================================================================================\r
