...
\r
// ============================================================================\r
// 1. Array-Based Stack Implementation (LIFO)\r
// ============================================================================\r
typedef struct {\r
    int items[MAX];\r
    int top;\r
...
\r
void initArrayStack(ArrayStack *s) {\r
    s->top = -1;\r
}\r
\r
bool isStackFull(ArrayStack *s) {\r
    return s->top == MAX - 1;\r
}\r
\r
bool isStackEmpty(ArrayStack *s) {\r
    return s->top == -1;\r
}\r
\r
void push(ArrayStack *s, int value) {\r
    if (isStackFull(s)) {\r
        printf("[STACK OVERFLOW] Cannot push %d. Stack capacity reached.\\n", value);\r
        return;\r
    }\r
    s->items[++(s->top)] = value;\r
...
int pop(ArrayStack *s) {\r
    ...
    int poppedVal = s->items[(s->top)--];\r
    printf("[STACK POP] Popped %d\\n", poppedVal);\r
    return poppedVal;\r
}\r
\r
int peekStack(ArrayStack *s) {\r
    if (isStackEmpty(s)) return -1;\r
    return s->items[s->top];\r
}\r
\r
// ============================================================================\r
// 2. Circular Queue Implementation (FIFO with Modulo Wrapping)\r
// ============================================================================\r
typedef struct {\r
    int items[MAX];\r
    int front;\r
...
\r
void initQueue(CircularQueue *q) {\r
    q->front = -1;\r
    q->rear = -1;\r
}\r
\r
bool isQueueFull(CircularQueue *q) {\r
    return (q->rear + 1) % MAX == q->front;\r
}\r
\r
bool isQueueEmpty(CircularQueue *q) {\r
    return q->front == -1;\r
}\r
\r
void enqueue(CircularQueue *q, int value) {\r
    if (isQueueFull(q)) {\r
        printf("[QUEUE FULL] Cannot enqueue %d (Front: %d, Rear: %d)\\n", value, q->front, q->rear);\r
        return;\r
    }\r
    if (isQueueEmpty(q)) {\r
        q->front = 0;\r
    }\r
    q->rear = (q->rear + 1) % MAX;\r
    q->items[q->rear] = value;\r
    printf("[ENQUEUE] Added %d at index %d\\n", value, q->rear);\r
}\r
\r
int dequeue(CircularQueue *q) {\r
    if (isQueueEmpty(q)) {\r
        printf("[QUEUE UNDERFLOW] Queue is empty.\\n");\r
        return -1;\r
    }\r
    int val = q->items[q->front];\r
    if (q->front == q->rear) { // Reset empty state\r
        q->front = -1;\r
        q->rear = -1;\r
    } else {\r
        q->front = (q->front + 1) % MAX;\r
    }\r
    printf("[DEQUEUE] Removed %d\\n", val);\r
    return val;\r
}\r
\r
void printQueue(CircularQueue *q) {\r
    if (isQueueEmpty(q)) {\r
        printf("Queue: [ EMPTY ]\\n");\r
        return;\r
    }\r
    printf("Queue Contents: [ ");\r
    int i = q->front;\r
    while (1) {\r
        printf("%d ", q->items[i]);\r
        if (i == q->rear) break;\r
        i = (i + 1) % MAX;\r
    }\r
...
`;function E(){const l=r.useRef([]),[i,p]=r.useState([12,28,45,67,89]),[s,u]=r.useState(0),[y,c]=r.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,m]=r.useState("");r.useEffect(()=>{const t=new IntersectionObserver(n=>{n.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return l.current.forEach(n=>{n&&t.observe(n)}),()=>t.disconnect()},[]);const a=t=>{t&&!l.current.includes(t)&&l.current.push(t)},x=()=>{u(t=>(t+1)%i.length),c(`Advanced to step ${(s+1)%i.length}: Invariant verified at index ${(s+1)%i.length}.`)},g=()=>{const t=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(t)||(p(n=>[...n.slice(-7),t]),c(`Pushed value ${t} into active memory structure. Buffer state updated.`),m(""))},f=()=>{p([12,28,45,67,89]),u(0),c("Structure reset to base state."),m("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
