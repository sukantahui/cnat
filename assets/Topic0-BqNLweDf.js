...
`;function P(){const l=a.useRef([]),[i,u]=a.useState([12,28,45,67,89]),[s,p]=a.useState(0),[x,c]=a.useState("Visualizer initialized. Ready for step-by-step state simulation."),[d,m]=a.useState("");a.useEffect(()=>{const t=new IntersectionObserver(n=>{n.forEach(o=>{o.isIntersecting&&o.target.classList.add("is-visible")})},{threshold:.08});return l.current.forEach(n=>{n&&t.observe(n)}),()=>t.disconnect()},[]);const r=t=>{t&&!l.current.includes(t)&&l.current.push(t)},g=()=>{p(t=>(t+1)%i.length),c(`Advanced to step ${(s+1)%i.length}: Invariant verified at index ${(s+1)%i.length}.`)},f=()=>{const t=d.trim()!==""?parseInt(d,10):Math.floor(Math.random()*90+10);isNaN(t)||(u(n=>[...n.slice(-7),t]),c(`Pushed value ${t} into active memory structure. Buffer state updated.`),m(""))},y=()=>{u([12,28,45,67,89]),p(0),c("Structure reset to base state."),m("")};return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
        .reveal-section {
          opacity: 0.99;
